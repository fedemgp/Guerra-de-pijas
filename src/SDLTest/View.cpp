...
View::View()
    ...
    if (this->window == nullptr) {
        this->close();
        throw Exception("Window could not be created! SDL_Error: %s", SDL_GetError());
    }
    ...
    if (this->renderer == nullptr) {
        this->close();
        throw Exception("Renderer could not be created! SDL Error:%s", SDL_GetError());
    }
    ...
    if ((!IMG_Init(imgFlags)) & imgFlags) {
        this->close();
        throw Exception("SDL_image could not initialize! SDL_image Error: %s", IMG_GetError());
    }
