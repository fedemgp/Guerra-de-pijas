...
void Weapon::PowerBar::update(float dt) {
    if (this->shotStarted) {
        this->elapsedTime += dt;
        if (this->power < POWER_FRAMES_QUANTITY && this->elapsedTime < POWER_CHARGE_TIME) {
            this->animations.emplace_back(this->textureManager.get(GUI::GameTextures::PowerBar),
...
void Weapon::PowerBar::render(GUI::Position &p, GUI::Camera &cam, SDL_RendererFlip &flip) {
    for (int i = 0; i < this->power; i++) {
        GUI::Position powerPos =
            GUI::Position((SCOPE_DISTANCE * (log10(10 * i / 17))) * cos(this->angle * PI / 180),
                          (SCOPE_DISTANCE * (log10(10 * i / 17))) * sin(this->angle * PI / 180)) +
            p;
