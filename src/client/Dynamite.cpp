...
Worm::Dynamite::Dynamite(const GUI::GameTextureManager &tex)
    : Weapon(tex, GUI::GameTextures::WormDynamite, DYNAMITE_CENTER_FRAME, WeaponID::WDynamite) {}

void Worm::Dynamite::update(float dt) {
    this->weaponAnimation.update(dt);
}

void Worm::Dynamite::render(GUI::Position &p, GUI::Camera &cam, SDL_RendererFlip &flip) {
