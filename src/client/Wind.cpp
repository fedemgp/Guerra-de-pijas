...
GUI::Wind::Wind(const GUI::GameTextureManager &tex, GUI::Camera &cam) : tex(tex), cam(cam) {}

void GUI::Wind::render(std::int8_t intensity, int windowWidth) {
    const GUI::Texture &toUse = (intensity > 0) ? this->tex.get(GameTextures::WindRight)
                                                : this->tex.get(GameTextures::WindLeft);
    float scaledIntensity = (float)std::abs(intensity) / 127 * this->cam.getScale();
    GUI::WrapTexture wt{toUse, scaledIntensity, (float)toUse.getHeight() / this->cam.getScale()};
