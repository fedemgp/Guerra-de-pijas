...
Worm::Holy::Holy(const GUI::GameTextureManager &tex) :
        Weapon(tex, GUI::GameTextures::WormHoly, HOLY_CENTER_FRAME,
               WeaponID::WHoly),
...
void
Worm::Holy::render(GUI::Position &p, GUI::Camera &cam, SDL_RendererFlip &flip){
...
void Worm::Holy::setAngle(float angle, Worm::Direction d){
    this->weaponAnimation.setFrame((int)std::ceil(angle / ANGLE_STEP) +
                                   this->centerFrame);
