...
Game::Config::Config(const YAML::Node &node) :
        jumpVelocity(node[JUMP][VELOCITY][X].as<float>(),
                     node[JUMP][VELOCITY][Y].as<float>()),
        ...
        banana(node[BANANA]),
        holy(node[HOLY]),
        aerialAttackMissileQuantity((std::uint8_t) node[AERIAL_ATTACK][BULLET][QUANTITY].as<unsigned int>()),
